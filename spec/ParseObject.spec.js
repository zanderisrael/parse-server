...
describe('Parse.Object testing', () => {
  ...
  it('many items added to an array at the same time - issue 6812', async () => {
    const obj = new Parse.Object('TestObject');
    const item = {
      amount_in_cents: 115,
    ...
    for (let i = 0; i < amount; i++) {
      obj.add('waitTimes', item);
      promises.push(obj.save());
    }
    await Promise.all(promises);
    await obj.fetch();
    expect(obj.get('waitTimes').length).toEqual(100);
    obj.get('waitTimes').forEach(waitTime => expect(waitTime).toEqual(item));
  });

  it('many items added to an array in sequence - issue 6812', async () => {
    const obj = new Parse.Object('TestObject');
    const item = {
      amount_in_cents: 115,
      duration_in_seconds: 210,
